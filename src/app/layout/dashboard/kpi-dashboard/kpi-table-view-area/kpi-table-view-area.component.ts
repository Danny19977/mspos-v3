...
export class KpiTableViewAreaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // route param 'province_uuid' holds the province UUID
    this.province_uuid = this.route.snapshot.params['province_uuid'] ?? '';
  ...
  load(): void {
    ...
    this.kpiService.TableViewArea(params).subscribe({
      next: res => {
  ...
  // Backend returns uuid = areas.uuid — group by it
  toggleSort(): void {
  ...
  openUserVisits(r: KPITableViewPriceModel): void {
    ...
    const el = document.getElementById('kpi-visits-offcanvas-area');
    if (el) {
